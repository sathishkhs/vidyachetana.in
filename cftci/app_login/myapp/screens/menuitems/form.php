	        ...
	        <form class="form-horizontals" method="post" id="menu_form" name="menu_form" action="master/menuitem_save" enctype="multipart/form-data">
	            ...
	            <div class="card-body">
	                        ...
	                        <div class="form-group">
	                            ...
	                            <select class="form-control" name="parent_menuitem_id" id="parent_menuitem_id" choosen="<?php echo (!empty($query->parent_menuitem_id)) ? $query->parent_menuitem_id : ''; ?>">
	                                ...
	                                <?php foreach ($menuitems as $row) : ?>
	                                    <?php if ($row->parent_menuitem_id == NULL) { ?>
	                                        ...
	                                        <?php foreach ($menuitems as $row2) { ?>
	                                            <?php if ($row2->parent_menuitem_id == $row->menuitem_id) { ?>
	                                                <option value="<?php echo $row2->menuitem_id; ?>">&nbsp;&nbsp; &raquo; <?php echo strtolower($row2->menuitem_text); ?></option>
	                                    <?php }
                                            }
                                        } ?>
	                                    <!-- <?php // }else{ 
                                                ?>
                                            <option value="<?php echo $row->menuitem_id; ?>" <?php echo (!empty($query->parent_menuitem_id) && ($row->menuitem_id == $query->parent_menuitem_id)) ? 'selected' : ''; ?>>&nbsp;&nbsp; &raquo; <?php echo strtolower($row->menuitem_text); ?></option>
                                    <?php // } 
                                    ?> -->
	                ...
	                <div class="row">
	                    ...
	                    <div class="col-xs-6 col-sm-6 col-md-6">
	                        <div class="form-group">
	                            <input name="menuitem_target" value="_blank" type="radio" <?php if (!empty($query->menuitem_target) && $query->menuitem_target == '_blank') {
                                                                                                echo 'checked';
                                                                                            }
                                                                                            ?> />
	                            <span class="lbl">New Window</span>
	                            <input name="menuitem_target" value="_self" type="radio" <?php if (empty($query->menuitem_target) || (!empty($query->menuitem_target) && $query->menuitem_target == '_self')) {
                                                                                                echo 'checked';
                                                                                            }
                                                                                            ?> />
