...
class Payments_model extends CI_Model
{
    ...
    public function view_donations($table,$from_date,$to_date){
        
         $this->db->select('*');
        if(!empty($from_date) && !empty($to_date)){

            $this->db->where('payment_date >= ', date('Y-m-d', strtotime($from_date)));
            $this->db->where('payment_date <=', date('Y-m-d', strtotime("+1 day",strtotime($to_date))));
    ...
    public function get_rp_payments($id)
    {
       $sql = "select * from (select * from payments union all select * from new_year_donations) a where razorpay_payment_id = '$id'";
