...
// 		$data['facilities'] = $this->master_model->get_rowdata('facilities');
		...
		$data['scripts'] = array('../js-plugins/summernote/summernote-lite.min.js','assets/javascripts/master.js');
		$this->load->view('templates/dashboard', $data);
	}
...
// 		}
	...
	public function impact_numbers(){
		$max_id = $this->master_model->max_id('impact_numbers','impact_numbers_id');
		$this->master_model->primary_key = array('impact_numbers_id',  $max_id );
		$data['query'] = $this->master_model->get_row('impact_numbers');
		// $data['categories'] = $this->project_model->get_categories();
		$data['view'] = 'impact_numbers/form';
		$data['title'] = 'Administrator Dashboard';
		$data['page_heading'] = 'Add Impact Numbers';
		$data['breadcrumb'] = 'Add Impact Numbers';
		$data['scripts'] = array('assets/javascripts/master.js');
		$this->load->view('templates/dashboard', $data);
	}

	public function impact_numbers_save(){
	
			$this->master_model->data['beneficiaries_served_daily'] = !empty($_POST['beneficiaries_served_daily']) ? $_POST['beneficiaries_served_daily'] : '';
			$this->master_model->data['beneficiary_hospitals'] = !empty($_POST['beneficiary_hospitals']) ? $_POST['beneficiary_hospitals'] : '';
			$this->master_model->data['schools_on_wheels'] = !empty($_POST['schools_on_wheels']) ? $_POST['schools_on_wheels'] : '';
			$max_id = $this->master_model->max_id('impact_numbers','impact_numbers_id');
			if($max_id > 0 ){
				$this->master_model->primary_key = array('impact_numbers_id', $max_id);
				if($this->master_model->update('impact_numbers')){
				...
				if($this->master_model->insert('impact_numbers')){
					$msg = array('type' => 'success', 'icon' => 'lni lni-thumbs-up lni-lg mr-2', 'txt' => 'Data Added Successfully');
				} else {
					$msg = array('type' => 'danger', 'icon' => 'lni lni-thumbs-down lni-lg mr-2', 'txt' => 'Sorry! Unable to save.');
				}
		...
		$this->session->set_flashdata('msg', $msg);
		redirect('master/impact_numbers');
